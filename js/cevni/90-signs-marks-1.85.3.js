...
(function(){
  ...
  function install(){
    ...
    // These early catalogue/prototype screens duplicate the current sign school.
    // Keep their source data available for audit, but retire the learner routes.
    document.querySelectorAll('#cevniPage > .cevniNav button').forEach(button=>{
      if(['VISUAL SCHOOL','DEEP TRAINING'].includes(button.textContent.trim()))button.remove();
    });
    if(typeof window.cevniScenarioAnswer==='function'){
      ...
      window.cevniScenarioAnswer=function(...args){
        ...
        document.querySelector('#cevniScenarioDebrief button[onclick*="cv1500Open"]')?.remove();
        return result;
      };
    }
