(function(){
'use strict';
const BUILD='1.69.0-M04-LIBRARY-PREVIEW';
...
const readyPromise=Promise.all([read('libraries/cevni/module04-briefings-1.69.0.json'),read('project-watch-cevni-question-bank-1.69.0.json'),read('libraries/cevni/module04-visuals-1.69.0.json')]).then(([lesson,bank,visuals])=>{
 if(lesson.build!==BUILD||visuals.build!==BUILD)throw Error('Module 4 library version mismatch');
 const live=bank.filter(q=>q.module===4&&q.liveInBuild===BUILD);
...
window.PW1690_M04={build:BUILD,source:'CEVNI Rev.6 Chapter 3 / Annex 3',phase:'library-preview',questionBankIntegrated:true,visualsAssessmentReady:false,module3Changed:false};
})();
