(function(){
 ...
 function source(key){const s=data.sources[key];return s?'<a href="'+s.url+'" target="_blank" rel="noopener noreferrer">'+s.label+' ↗</a>':''}
 function showLesson(i,scroll=true){
  ...
  root.innerHTML='<div class="homeEyebrow">LESSON '+String(i+1).padStart(2,'0')+' / '+data.lessons.length+'</div><h2>'+l.title+'</h2><p class="pwSafetyLead">'+l.lead+'</p><img src="'+l.image+'" alt="'+l.alt+'" loading="lazy"><div class="pwSafetyPoints">'+l.points.map(p=>'<div class="pwSafetyPoint"><h3>'+p[0]+'</h3><p>'+p[1]+'</p></div>').join('')+'</div><div class="pwSafetyScenario"><b>ON THE WATER</b><p>'+l.scenario+'</p></div><div class="pwSafetyActions"><button type="button" id="pwSafetyMark">MARK LESSON REVIEWED ✓</button><button type="button" class="secondary" id="pwSafetyNext">'+(i<data.lessons.length-1?'NEXT LESSON →':'KNOWLEDGE CHECK →')+'</button></div><div class="pwSafetySourceRef">Further reading: '+source(l.source)+'</div>';
