(function(){
 ...
 function showLesson(i,scroll=true){
  ...
  root.innerHTML='<div class="homeEyebrow">LESSON '+String(i+1).padStart(2,'0')+' / '+data.lessons.length+'</div><h2>'+l.title+'</h2><p class="pwSafetyLead">'+l.lead+'</p><figure class="pwSafetyScene"><img src="'+l.scene+'" alt="'+l.sceneAlt+'" loading="lazy"><figcaption><b>SEE THE SITUATION</b><span>'+l.sceneCaption+'</span></figcaption></figure><div class="pwSafetyTeachingLabel">WHAT TO DO</div><div class="pwSafetyPoints">'+l.points.map((p,j)=>'<div class="pwSafetyPoint"><span class="pwSafetyPointNo">0'+(j+1)+'</span><h3>'+p[0]+'</h3><p>'+p[1]+'</p></div>').join('')+'</div><div class="pwSafetyScenario"><b>PUT IT INTO PRACTICE</b><p>'+l.scenario+'</p></div><div class="pwSafetyActions"><button type="button" id="pwSafetyMark">MARK LESSON REVIEWED ✓</button><button type="button" class="secondary" id="pwSafetyNext">'+(i<data.lessons.length-1?'NEXT LESSON →':'KNOWLEDGE CHECK →')+'</button></div>';
