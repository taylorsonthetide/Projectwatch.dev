...
The current register is **not a complete plate index**. Annex 7 contains 33 further distinct variant or sign codes absent from its 99 records:

- A: `A.1a–f`, `A.9a–b`, `A.11a–c`.
- B: `B.12` (obligation to use an onshore power supply point).
- C: `C.1a–b`, `C.2a–b`, `C.3a–b`, `C.5a–b`.
- E: `E.1a–d`, `E.5.7–E.5.15` (additional reserved berthing classes).

Some are alternate presentations of an umbrella meaning already listed (for example A.1 and C.1); they need distinct image records if users are expected to recognise each plate. B.12 and the extra E.5 classes add meanings that the current register omits. The umbrella headings B.2, B.3, B.4, B.9, D.1, D.2, D.3, E.4, E.9–E.12 are not additional pictograms by themselves.
